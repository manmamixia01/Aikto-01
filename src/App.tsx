    ...
    <motion.button
      ...
      animate={{
        backgroundColor: isClicked 
          ? (isWhite ? "#000000" : "#ffffff") 
          : (isWhite ? "#ffffff" : "transparent"),
        color: isClicked 
          ? (isWhite ? "#ffffff" : "#000000") 
          : (isWhite ? "#000000" : "#ffffff"),
        borderColor: isClicked ? (isWhite ? "#000000" : "#ffffff") : (isWhite ? "#ffffff" : "rgba(255, 255, 255, 0.2)")
      }}
      className={`px-8 py-4 font-bold transition-shadow ${className}`}
    >
      {children}
    </motion.button>
  );
...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    const initMagnetic = () => {
      ...
      if (isTouchDevice) return; // Disable magnetic on touch devices (iPad, etc.)
    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "glass-nav py-4" : "bg-transparent py-6"}`}>
      <div className="full-width-container flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link to="/" className="text-2xl font-bold tracking-tighter font-display magnetic">
            AKITO
          </Link>
          <div className="hidden md:flex items-center gap-6 text-sm font-medium tracking-wide text-white/70">
            {navLinks.map((link) => (
              <a 
                ...
                className="hover:text-white transition-colors nav-link-hover magnetic"
              >
                {link.name}
              </a>
            ))}
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <AnimatedButton 
            className="flex items-center gap-1 md:gap-2 text-[10px] md:text-sm px-3 py-1.5 md:px-5 md:py-2 whitespace-nowrap"
          >
...
const Hero = ({ lang, show }: { lang: 'en' | 'jp', show: boolean }) => {
  ...
  return (
    <section id="about" className="relative h-[100dvh] w-full overflow-hidden flex items-start pt-28 md:pt-48">
...
const kineticFitTranslations = {
  en: {
    ...
    steps: [
      ...
      { id: "06", title: "Skeletal Map", desc: "Calculate full skeletal structure by integrating all joint data.", image: "https://github.com/manmamixia01/Web-public/blob/main/6.png?raw=true" },
...
const KineticFitPage = ({ lang }: { lang: 'en' | 'jp' }) => {
    ...
    <div className="min-h-screen bg-bg-dark pt-32 pb-20 px-6 font-sans overflow-x-hidden">
      <div className="full-width-container">
        ...
        <motion.div 
          ...
          animate={{ opacity: 1, y: 0 }}
          whileTap={{ scale: 0.95 }}
          className="flex items-center gap-4 mb-20 group cursor-pointer w-fit"
          onClick={() => navigate('/')}
        >
          <div className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:text-black group-active:bg-brand-orange group-active:text-white transition-all duration-500">
            <ArrowLeft size={18} />
          </div>
        ...
        <div className="pt-20 border-t border-white/10 text-center">
           ...
           >
              <div className="w-20 h-20 rounded-full border border-white/10 flex items-center justify-center group-hover:bg-white group-hover:text-black group-active:bg-brand-orange group-active:text-white transition-all duration-700 ease-expo">
                 <ArrowLeft size={32} />
              </div>
              <div>
              ...
              <section className="py-32 px-6 text-center border-t border-white/10">
                <div className="max-w-4xl mx-auto">
                  ...
                  <div className="flex flex-wrap justify-center gap-6">
                    <AnimatedButton className="text-lg px-10 py-5">
